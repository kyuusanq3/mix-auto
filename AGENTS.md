...
- **Local LLM docs shape (2026-07):** Prefer golden Kotlin DO/DON'T snippets in topic rules over long bullet lists; use root `llms.txt` as package index; soft under-~300-line budget for new collaborators (cohesive ~350 OK if split breaks a bug class); for files over ~300 lines, signature/grep skim before full body (`local-map-triage` Step 3b).
...
- **Cursor `/local-opencode` (2026-08):** Personal skill `~/.cursor/skills/local-opencode` replaces `/local-llm` (redirect stub remains). Conductor must **run** non-interactive `opencode run --auto` via `scripts/run-opencode.ps1` (prompt after `--`; do **not** use `--file` for the message on Windows — misparses as a path). Not paste-brief-and-stop. Map `mixauto-*` → OpenCode IDs via `recommend-model.ps1` (`OPENCODE_MODEL=`).
- **Unattended OpenCode camera drift (2026-08):** Headless 30B invented a phantom speed-HUD bug and edited `LocationTrackingController`; 14B nested a new function inside `targetZoomForManeuverDistance`. Kill + `git checkout` on drift; prefer one-turn prompts and proof-grep before claiming done.
...
- **OpenCode false-success + test-compile gap (2026-08):** Speed-zoom Turn C @14B failed `Edit`/`oldString` but claimed `minOf` done while verify stayed green; Turn A left `NavigationZoomTest` missing class `}` — `assembleDebug` did not catch it. Hardening: conductor `/local-opencode` Step 5 proof (grep live symbols) + Step 6 escalate on Edit fail/missing proof; retry with **exact current→new body** prompts (Windows Edit mismatches); `verify-debug.ps1 -UnitTestCompile` for Turn A / `app/src/test/**`; proof table in `local-map-feature`.
...
- **Cursor `/local-opencode` dry-run skip (2026-08):** After script-first patch dry-run prints `PATCH_DONE`/`ALREADY_PATCHED` and proof symbols are present → **skip OpenCode**; conductor runs `./scripts/verify-debug.ps1` (skill Step 3b/4b). Spawning a full re-apply OpenCode session after a green dry-run often idle-kills (`OPENCODE_KILL=IDLE` exit 51) before first tool use. Gate helper: `~/.cursor/skills/local-opencode/scripts/after-patch-gate.ps1` → `OPENCODE_GATE=SKIP_VERIFY_CONDUCTOR`; ScriptOnly idle default **120s**.
- **OpenCode Edit-match / widened-gate / path-guess (2026-08):** DeveloperSettings zoom run — 14B created the object + docs but failed MapSettings Zoom wrap (`oldString`), guessed `ui/settings/MapSettingsPanel.kt`, wrapped all of `DrivingViewSettingsSection` (broke compile), then verified green and claimed success without the Zoom gate; 30B drifted into `NAV_TILT_OFFSET`. Hardening: `build.txt` / `local-opencode-loop` **PROOF BEFORE VERIFY** + Edit-fail (retry once then STOP) + path/scope/docs-table rules; conductor Step 6 **script fallback** (temp `.ps1` needle) before 30B; drift kill if first tools leave named owners; escalate with **shorter** locked prompts.
- **OpenCode Unicode string-split Edit (2026-08):** POI search source label — 14B Edit landed but rewrote `"$distanceLabel · $sourceLabel"` as `"$distanceLabel" · "$sourceLabel"` (`Unexpected tokens`), then hung without verify; first script-fallback needle used a literal `·` and missed (encoding). Hardening: conductor prompts prefer ASCII/`a + " · " + b`; Step 5/6 treat compile-on-owner + hung-after-Edit as fail → **immediate script fallback** with `\u`/regex needles (never literal fancy glyphs in `.ps1`); `build.txt` / `local-opencode-loop` Edit-fail covers invalid syntax after “successful” Edit.
...
- **Zed qwen 30b rubber-band derail (2026-07-31):** Thread in `%LOCALAPPDATA%\Zed\threads\threads.db` (zstd) — “Rubber-band Puck GPS Smoothing Lag Fix” (`mixauto-qwen3-coder:30b`, MixAuto Write). Loaded skills + grepped skill/docs hits (~14–55k), ask-looped, then after “execute now” nudge ran `ls`, chased `opencode.json` / `text-pitch-alignment` / wrong `data/map/mix-auto-driving.json`, never edited `SmoothingLocationEngine.kt`. Cause: skill-dump context drowning + `/map-bug` mention injecting label recipes into a puck task. Diminishing returns on more `.md` hardening — capacity/context flooding dominates.
- **Big→small local pipeline (2026-08):** Feasible shape = user one-liner → assessor (prefer Cursor over DeepSeek) fills a **locked** handoff (class, ≤3 owners, one edit target, forbidden, verify) → Qwen gets **only** that short prompt (no skill dump / no `/map-bug` body). DeepSeek freeform “prompt making” often becomes multi-bucket plan poison. Auto-tuning Ollama `PARAMETER` (temp/`num_ctx`) from failure signals is DIY-only and low ROI vs wrong-owner/semantics bugs. Zed does not chain models natively — paste or external orchestration.
...
- **Cursor `/local-opencode` junior pipeline (2026-08):** Treat OpenCode as apply+verify junior under Cursor conductor. **Script-first** for ≥2 hunks/files / new files / after one Edit fail — copy `~/.cursor/skills/local-opencode/scripts/eol-safe-patch.template.ps1`, use `PatchHelpers.ps1` (EOL-normalized needles). Launcher `run-opencode.ps1` watch/kill: exit **50** forbidden skill/run-tool invocation, **51** idle, **52** max time, **53** early no `-OwnerHint`; use `-ScriptOnly` + `-OwnerHint`; `-AllowPitchTools` only for real label-stretch. Proof `Select-String` patterns ASCII-only (no `&&`). Do not trust OpenCode prose without conductor proof-grep.
