# MixAuto � Agent Guide
## /local-opencode overlay

Conductor settings for OpenCode turns (verify, model, temp dir, skills): [`LOCAL-OPENCODE.md`](LOCAL-OPENCODE.md). Do not duplicate those fields into the personal `local-opencode` skill.
...
| In-nav grey next-turn fork (OSRM, ±5 min, adopt if taken) | `ManeuverAlternatePlanner`, `ManeuverAlternateCoordinator`, `OffRouteDetector`, `RouteRenderer.showManeuverAlternate` | `mix-auto-navigation.mdc` |
...
| Offline POI DB, offline map tiles | `LocalPlacesRepository`, `LocalPlacesSearch`, `OfflineMapRepository`, `OfflineRegionDownloadSession`, `OfflineProgressFormatting`, `MapDataOverlay` | `mix-auto-places.mdc` |
...
- **Dashboard UI decomposition:** Portrait/landscape dock layouts in `DashboardLayouts.kt`; prop holders in `DashboardLayoutProps.kt`; secondary pane in `DashboardSecondaryPane.kt`; side-effects in `DashboardScreenEffects.kt`; panel-session lambdas in `DashboardPanelActions.kt`.
...
- **DeveloperSettings hardcoded flags (2026-08):** `ui/settings/DeveloperSettings.kt` � compile-time toggles (not prefs/UI). `MANUAL_DRIVING_ZOOM` **false** (default) = hide Map Settings Zoom slider + free-drive speed follow-zoom; **true** = show slider + skip free-drive speed curve. `SHOW_POI_SOURCE` **false** = hide source next to search-row distance; **true** = show `overture|vector|�`. `FILTER_LOW_CONFIDENCE_POIS` **true** + `MIN_POI_CONFIDENCE` **0.5f** = SQL-filter Overture rows in `LocalPlacesSearch`. `APPROXIMATE_POI_CONFIDENCE_CEILING` **0.7f** = show approximate icon when Overture `confidence` is set and (`!hasStreetAddress` or conf below ceiling). Prefs/`setDrivingZoom` stay wired for manual. `SHOW_DOCK_MUSIC_SIDE_CONTROL` **false** (default, v0.0.43) = hide `DockMusicSideControl` on the shortcut bar. Agents: add future developer flags here; grep `DeveloperSettings`.
...
- **Offline POI SQL LIMIT before distance (2026-08):** Symptom: SM/Savemore search shows only ~40+ km hits. `FILTER_LOW_CONFIDENCE_POIS` / `MIN_POI_CONFIDENCE` 0.5 is usually **not** the cause (most brand rows pass). Root cause: `LocalPlacesSearch.searchWithFts5` / `searchWithLike` applied `LIMIT` (`LOCAL_RESULT_LIMIT` = 15) inside `BBOX_DELTA` +/-0.5 deg **before** distance sort � dense queries return arbitrary far candidates; Kotlin `rankSearchResults` only reorders that set. Fix: pass origin lat/lng and `ORDER BY ((lat - ?) * (lat - ?) + (lng - ?) * (lng - ?))` then `LIMIT`. Diagnose against `mix-auto-overture-maps/places-dist/ph_places.db` (or installed `files/places/*.db`). Session: `C:/dev/skills/session-history/mix-auto/poi-search-limit-before-distance_2026-08-07_0515.md`.
- **Dashboard status strip direction (2026-08):** `DashboardStatusBar.kt` is low-value always-on chrome for driving; if the top strip is removed, prefer moving **time + weather** into the `MediaPlayerPane` header rather than the map toolbar/dock, and replace the traffic marquee with a compact severity-colored badge or chip instead of another full-width reel. `CarMapViewContainer.kt` already uses the prime map overlay slots for nav HUD, search, map tools, and speed.
- **`/local-opencode` model + proof order (2026-08):** For mix-auto coding turns, prefer `ollama/qwen3-coder-14b-24k` over `ollama/freehuntx/qwen3-coder:14b` — the latter can hit an OpenCode ~8k context trap despite larger Ollama metadata. Junior prompts must use **literal absolute temp paths** (not `$env:TEMP` / `%TEMP%`) and must proof-grep **only after** patch apply returns `PATCH_DONE` / `ALREADY_PATCHED`; pre-apply `Select-String` results are expected-empty and not proof failure.
- **Minimized audio sidebar layout (2026-08):** Owner `AudioPlayerMinimizedSidebar.kt`; width via `DashboardScreen.kt` `AUDIO_PLAYER_MINIMIZED_MEDIA_WEIGHT` (**0.09f** = 1.5× former 0.06f). Use `SECTION_GAP` (16 dp) between time / date / weather / traffic / visualizer / album art blocks and `ITEM_GAP` (2 dp) inside each block. Clock: `DateTimeFormatter` patterns **`hh`** + **`mm`** (fixed 2-digit). Date column: **day + month only** (no year). Weather: icon + temp in one `Row(ITEM_GAP)`; traffic stays standalone `Icon`. Bottom block: weighted `Column` — `DockMiniVisualizer(wide = true, height = 56.dp)` then album art in `BoxWithConstraints` sized `minOf(maxWidth, maxHeight)` — **do not** use `fillMaxWidth().aspectRatio(1f)` (overflows narrow sidebar, paints under shortcut dock, hides visualizer). No chevron — whole sidebar/bar is `clickable(onToggleMinimized)`.
- **`/local-opencode-t1a` conductor contract (2026-08):** When user attaches T1A, host is **conductor only** — write ≤80-line spec, `spec-gate.ps1`, spawn `run-opencode.ps1 -EditOnce`; **do not** `StrReplace`/`Write` app source (same failure class as TSK-009). Split multi-owner requests into one phase per turn (width weight vs sidebar layout). On **`OPENCODE_KILL` exit 56** (`NO_OUTPUT_BEFORE_STARTUP_TIMEOUT`): (1) test Ollama directly — `Invoke-RestMethod http://localhost:11434/api/generate` with trivial prompt; if timeout, restart `ollama.exe` (wedged queue shows loaded model + near-zero CPU); (2) if Ollama OK but OpenCode still silent 300s, suspect session prefill (auto-loaded skills + AGENTS/rules) — do not blind-retry; user may fall back to direct edit or T0. See session `minimized-audio-bar-t1a-opencode-debug_2026-08-21_2226.md`.
- **Audio pane minimize/maximize without chevrons (2026-08):** Chevrons removed from minimized sidebar (`AudioPlayerMinimizedSidebar.kt`) and maximized glance strip (`MediaSessionGlanceWidget.kt`). **Minimize:** drag `MapMediaDividerHandle` past clamp — `rawRatio > 0.8f` through `updateMapMediaRatio` sets `isAudioPlayerMinimized = true` without persisting overshoot (`LauncherViewModel.kt`). **Maximize:** tap minimized sidebar/bar (`clickable` on root); `toggleAudioPlayerMinimized()` snaps to **0.8** via `updateMapMediaRatio(0.8f)` when expanding (smallest non-minimized split). Min maximized split = `coerceIn(0.3f, 0.8f)` max map; minimized weight `AUDIO_PLAYER_MINIMIZED_MEDIA_WEIGHT = 0.09f` in `DashboardScreen.kt`; divider hidden while minimized.
- **T1A literal REPLACE spec (2026-08):** For 3–20 line Kotlin/Compose body fills, **14B + exact FIND→newString block** beats paraphrased CHANGE bullets and **30B** (stock/mixauto often Read-then-essay with `OPENCODE_EXIT=0` and no Edit). Put likely wrong paths in FORBIDDEN (e.g. `DeveloperSettings.kt` when owner is `LauncherViewModel.kt`). `-ForbiddenSubstring` for map skills: use unquoted `local-map-triage`, not quoted `Skill "local-map-triage"`. Session: `C:/dev/skills/session-history/mix-auto/audio-chevron-t1a-phase-chain_2026-08-21_2225.md`.
- **In-nav maneuver forks (2026-08, v0.0.43):** On Navigate, `ManeuverAlternateCoordinator.prefetch` fetches OSRM `alternatives=3` from each main turn to dest; keep ±**5 min** vs remaining ETA and distinct from original **suffix** (`remainingGeometryFromManeuver` — do not compare against the full original polyline or the remaining path is kept as a fake grey overlay). During nav, grey `ROUTE_OSRM_ALT_*` only for the **next** step; `OffRouteDetector` treats that line as on-route; adopt when `distToPrimary > 35 m` and `distAlt ≤ 25 m`. Full contract: `mix-auto-navigation.mdc`. Session: `C:/dev/skills/session-history/mix-auto/nav-maneuver-forks-minimized-audio_2026-08-21_2225.md`.
- **Interrupted `/local-opencode` ctor merge (2026-08):** API-limit / kill mid-apply on `MapLibreEngineImpl` left a complete one-liner `OffRouteDetector(...)` **plus** leftover named args (will not compile). User “apply the fix yourself” exits conductor — host may `StrReplace`. Stale OpenCode jobs can still finish after the host repair (`OPENCODE_KILL` 52, old `BUILD FAILED` summaries); do not re-apply or treat those as live tree state.
- **File-size wave 5 (2026-08):** Extract a **named second job**, keep the public API on the owner, forward. Places: `LocalPlacesDownloader` (install) + `LocalPlacesSearch` (FTS5/LIKE/bbox; `ORDER BY` then `LIMIT`). Media: `MediaSessionParsers` (title/art/like/shuffle) — boot/resume stay on `MediaSessionRepository`. Prefs: `LauncherMapPrefs` / `LauncherAudioPrefs`; ViewModel still exposes the same setters so `MainActivity` method refs work. `updateMapMediaRatio` stays on the ViewModel (coupled to `isAudioPlayerMinimized`). UI: `MapHudOverlays`, `OfflinePackRowActions`, `DashboardScreenEffects`, `DashboardPanelActions`, `DashboardLayoutProps`. Do **not** size-split camera/GPS/smoothing/`pointerInput`/the facade. Named leftover: `MapLibreEngineImpl` dead reckoning only. Session: `C:/dev/skills/session-history/mix-auto/file-size-refactoring-extract-wave_2026-08-21_2227.md`.
- **Portrait minimized audio bar (2026-08):** Portrait minimize = full-width horizontal strip (~9% height), not the landscape sidebar — `PortraitMinimizedBar` in `AudioPlayerMinimizedSidebar.kt` early-returns when `isPortrait`; compact `Row` (time, date, weather, traffic, 48×24dp visualizer); no chevron, whole row clickable.
- **Google Maps share to Mix Auto (2026-08):** Symptom: Share sheet opens Mix Auto (`singleTask` + `onNewIntent`) but no POI drawer. Root cause: named business/POI shares (`"Place Name\nhttps://maps.app.goo.gl/xxxx"`) redirect to URLs with only a Google feature ID (`!1s0x...:0x...`) — no `@lat,lng`, `!3d/!4d`, or `/maps/place/<name>/` segment — so coordinate regexes in `GoogleMapsShareParser` miss and `pendingSharedPlace` stays null. Fix chain: mobile Chrome UA for short-link 302s (default `HttpURLConnection` gets interstitial, not `Location` header) → desktop UA body fetch for inline coords → `WebViewCoordinateResolver` (JS-rendered page) → Photon geocode of `extractNameHint()` / URL place name (`MapHostViewModel.handleSharedIntent`). Wiring: `MainActivity.onCreate`/`onNewIntent` → `MapHostViewModel` → `DashboardScreen` `LaunchedEffect(pendingSharedPlace)` → `focusOnPoi`. Log tag `MixAutoShare`. Photon/OSM fallback cannot resolve Google-only businesses with no OSM row. Session: `C:/dev/skills/session-history/mix-auto/google-maps-share-poi-fix_2026-08-21_2230.md`.
- **Puck placement lost on nav transitions (2026-08):** Symptom: puck snaps to screen center on reroute, some nav turns, exit nav despite Map Settings offset saved. Root cause: `paddingWhileTracking()` is **silently ignored** while `LocationComponent` camera mode is mid-transition — flipping `component.cameraMode = TRACKING_GPS` then immediately calling `applyDrivingTrackingPadding()` drops the saved offset. Fix: `NavigationCameraController.engageTrackingGpsWithPuckPadding()` uses `setCameraMode(TRACKING_GPS, OnLocationCameraTransitionListener)` and re-applies padding in `onLocationCameraTransitionFinished`/`Canceled`; wired from `activateFreeDriveTrackingMode`, `snapCameraToGpsIfNeeded`, `activateNavigationTracking`. Session: `C:/dev/skills/session-history/mix-auto/puck-padding-camera-transition-fix_2026-08-21_2229.md`.
