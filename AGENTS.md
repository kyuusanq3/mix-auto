...
**Saved places:** Up to 50 in `LauncherPreferences.savedPlaces` (same JSON schema + `isDroppedPin`). `LauncherViewModel.toggleSavedPlace()` dedupes at 50 m. Starred POIs show gold teardrop icons on map via `mapEngine.setSavedPlaces()` synced from `DashboardScreen`. Empty search field shows **Recent + Nearby** suggestions; header **star toggle** switches to saved-only list (typed search filters saved locally). Each row has a star button. **Add from link:** **+** icon in Destination Search header → paste Google Maps share link or coordinates → Confirm → custom pin draft + POI detail (**star to save**; map tap cancels without saving). **Short map tap** selects saved pin, active custom pin, or POI pin (`handleMapPointSelection()`). **Long press** on empty map drops a custom pin (`startCustomPinDraft()`); long press in free-drive GPS tracking exits to top-down view at the drop point. POI detail opens in media pane via `ActivePanel.POI_DETAIL`.
...
| Offline map download stuck at 0% / endless Preparing | Loopback HTTP style via `prepareOfflineRegionStyleUri()` (not `asset://`/`file://`); minimal `mix-auto-offline-pack.json`; legacy regions without `styleTransport: loopback-v1` are not resumed — delete (✕) and retry; prepare can take 1–2 min; timeout surfaces error via `handleDownloadFailure()` |
| Offline map stuck at 1–5% on phone after update | z14→z17 **Update** re-downloads entire region (150–400+ MB) — resource-count % can lag while bytes download; UI now shows **~X MB of ~Y MB** and bar uses `displayProgress` (max of resource% and byte%); not necessarily broken if MB keeps increasing; use Wi‑Fi, keep app foreground |
| Offline map prepare timeout / silent failure | Fixed: service calls `reportRegionDownloadError` + `pauseDownload`; Logcat `OfflineMapRepository` / `MapStyleAssetResolver`; TileJSON cached at `filesDir/map/planet-tilejson.json` |
| Blank map tiles while driving at z17 with offline pack | Regional packs must be **z17** (Map Data → **Update** if installed at z14); driving zoom 17.5 needs z15+ tiles — z14-only packs overzoom online; `DrivingTilePrefetcher` + ambient cache help while online |
| Map Data shows Installed but offline label missing | `hasCompleteRegionCovering()` requires current-detail pack — tap **Update** after app upgrade if subtitle shows "Installed z14" |
| `putResourceWithUrl` compile error in DrivingTilePrefetcher | MapLibre 11.5.1 uses 6-arg synchronous `putResourceWithUrl(url, data, modified, expires, etag, mustRevalidate)` — no callback parameter |
| Offline map prepare >120s | `PREPARE_TIMEOUT_MS` is 120s — UI shows timeout error; check Wi‑Fi to `tiles.openfreemap.org`; cached TileJSON fallback in `filesDir/map/planet-tilejson.json`; Logcat tag `OfflineMapRepository` |
...
| Import Google Maps POI | Destination Search → **+** beside star → paste share link (`maps.app.goo.gl` needs network) or `lat, lng` → Confirm; short link expands via `GoogleMapsLinkParser` (`formatPlaceDisplayName` — primary name only, `+`/`%20` decoded); **star in POI detail to save** (draft grey pin until starred; map tap dismisses without saving); blocked during navigation |
| Add Place "Could not find coordinates" on goo.gl | Mobile share links redirect to place-ID-only URLs — parser reads HTML body for `!3d`/`!2d` protobuf coords; needs network; paste plain `lat, lng` or desktop share URL with `@lat,lng` as fallback |
...
| Parallel streets blob when zoomed out | Split zoom ramps in `gen_mix_auto_driving_style.py` + `applyAutomotiveRoadBoost()` — main stock until z **13**, minor until z **15**, full boost at z **17**; factors **2.75×/3.1×** (JSON) + **1.55×/1.65×** (runtime); tune `MAIN_/MINOR_BOOST_ZOOM_START` and factors, then `python tools/gen_mix_auto_driving_style.py` |
...
| Puck still flickers while following GPS | Do NOT duplicate `requestLocationUpdates` on `rawLocationEngine` — app logic uses `BearingEnrichedLocationEngine.onRawFix`; skip `forceLocationUpdate` during active smoothing; bucketed lookahead padding; compare padding key in `handleMapLayoutChange` before force |
| Puck pauses/jumps between GPS fixes | Dead-reckoning `forceLocationUpdate` loop disabled (native render crash); free drive relies on LocationEngine + `SmoothingLocationEngine`; ensure `hasSpeed()` on fixes for bearing; head-unit gap between 1 Hz fixes is expected |
...
| Puck spins when stopped in traffic | `BearingEnrichedLocationEngine` must wrap fused engine for LocationComponent — `enrichLocationWithBearing()` alone in `applyAndroidLocation()` does not reach puck/camera; stopped = speed < 1.4 m/s or movement < 8 m; frozen bearing via `lastStableBearing` / camera bearing |
...
| Update download snackbar flashes | Do not dismiss/re-show Snackbar per progress tick — use `AppUpdateDownloadBanner`; throttle `AppUpdateViewModel` progress to 1% steps (`downloadApk` emits every buffer read) |
...
| Crash opening destination search (Nearby) | `CalledFromWorkerThreadException` on `latLngForPixel` — `getNearbyPois()` reads viewport via `currentViewportBounds()` which must run on main thread; `readOnMainThread()` in `MapLibreEngineImpl` |
| Route overview Logcat `paddingWhileTracking` spam | Benign SDK errors when called during `CameraMode.NONE`; fixed — `fitRouteOverviewCamera()` uses map padding only; `applyPaddingWhileTrackingIfEngaged()`; throttle overview re-fit to map size changes |
...
| Free drive camera doesn't follow puck | `applyDrivingTrackingPadding()` must skip `moveCamera(paddingTo)` when `cameraMode == TRACKING_GPS` — use `paddingWhileTracking` only; skip `forceLocationUpdateForImmediateRender()` while smoothing active; `applyAndroidLocation()` re-calls `activateFreeDriveTrackingMode()` only when tracking lost; clear `isInTopDownView` in `recenterCamera()` (before snap), `dismissSelectedPoi()`, and cancel `topDownViewportSync` in `startFreeDrive()` |
| Puck centered after top-view recenter | Top-down clears padding cache but not `paddingWhileTracking` in NONE mode — `recenterCamera()` → `scheduleFreeDrivePaddingRestore()` + `snapCameraToGpsIfNeeded` force puck refresh (`allowDuringSmoothing=true`); `activateFreeDriveTrackingMode` re-applies when `lastEngagedTrackingPadding == null`; common on full-map layout resize |
