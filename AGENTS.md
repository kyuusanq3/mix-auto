...
- **DeveloperSettings hardcoded flags (2026-08):** `ui/settings/DeveloperSettings.kt` — compile-time toggles (not prefs/UI). `MANUAL_DRIVING_ZOOM` **false** (default) = hide Map Settings Zoom slider + free-drive speed follow-zoom; **true** = show slider + skip free-drive speed curve. `SHOW_POI_SOURCE` **false** = hide source next to search-row distance; **true** = show `overture|vector|…`. `FILTER_LOW_CONFIDENCE_POIS` **true** + `MIN_POI_CONFIDENCE` **0.5f** = SQL-filter Overture rows in `LocalPlacesRepository`. Prefs/`setDrivingZoom` stay wired for manual. Agents: add future developer flags here; grep `DeveloperSettings`.
